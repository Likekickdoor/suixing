...
class leave_ajaxj {
    ...
    public function leave_display(){
        //var_dump($_GET['leave_id']);
        if(empty($_GET['leave_id'])){
            
        }else{
            $sql = "SELECT * FROM management_leave where id=".$_GET['leave_id'];
            $r_leave = $this->pdosqlObject->select_sql($sql);
            
            $this->pdosqlObject->update("management_leave",array("is_watch"=>1),array("id"=>$_GET['leave_id']));
