...
class TrainsPrice
{
	...
	*去除第一个不合理的价格的排序
	*/
	function VaildPriceSort($trainsTable){
		$First=['G','D','C'];
		$Second=['K','T','Z','Y','S','P'];
		if(count($trainsTable)>=2){
			$train_type_first=mb_substr(strtoupper($trainsTable[0]['trainNo']), 0,1);			
			if(in_array($train_type_first,$First)){
				//如果第一个的车次是G,D,C且后面有K,T,Z就不合理,交换
				foreach ($trainsTable as $key => $train) {
					if($key==0){
						continue;
					}
					$train_type=mb_substr(strtoupper($train['trainNo']), 0,1);
					if(in_array($train_type, $Second)){
						$temp_train=$trainsTable[0];
						$trainsTable[0]=$trainsTable[$key];
						$trainsTable[$key]=$temp_train;
						return $trainsTable;
					}
				}
				return $trainsTable;
	...
	*/
	function PriceUpSort($trainsTable){
	  for($i=0;$i<count($trainsTable)-1;$i++)
	  {
	  	...
	  	// $this->VaildPriceSort($trainsTable);
		return $trainsTable;
	}
	/**
	*按时间升序排列
